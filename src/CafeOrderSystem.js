...
const CafeOrderSystem = () => {
    ...
    const addOrder = () => {
        ...
        if (newOrder.drink.trim() !== '' && newOrder.size.trim() !== '' && newOrder.price.trim() !== '') {
    ...
    return (
        <div>
            ...
            {/* 새로운 주문 정보 입력 */}
            <input type="text" placeholder="음료" value={newOrder.drink} onChange={e => setNewOrder({ ...newOrder, drink: e.target.value })} />
            <input type="text" placeholder="사이즈" value={newOrder.size} onChange={e => setNewOrder({ ...newOrder, drink: e.target.size })} />
            <input type="text" placeholder="가격" value={newOrder.price} onChange={e => setNewOrder({ ...newOrder, drink: e.target.price })} />
